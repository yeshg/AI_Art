...
def deepdraw(net, base_img, octaves, random_crop=True, visualize=True, focus=None,
    ...
    for e,o in enumerate(octaves):                          # scale up image if specified in octaves
        ...
        cv2.imshow('Main',deprocess(net, image))
        cv2.waitKey(5)
...
# Declare windows and trackbars
cv2.namedWindow('Main', cv2.WINDOW_NORMAL)
cv2.createTrackbar('ClassToVisualize','Main',0,999,nothing)
cv2.createTrackbar('Quality','Main',1,3,nothing)
cv2.createTrackbar('Refinement','Main',0,50,nothing)
cv2.createTrackbar('Draw','Main',0,1,nothing)

# Set initial trackbar positions
cv2.setTrackbarPos('ClassToVisualize','Main', 79)
cv2.setTrackbarPos('Quality','Main', 2)
cv2.setTrackbarPos('Refinement','Main', 2)

#gen_image = np.random.normal(background_color, 8, (original_w, original_h, 3))
gen_image = np.random.normal(np.float32([28.0, 84.0, 122.0]),8,(original_w, original_h, 3))
base_img = gen_image
random_crop = True
visualize = True
clip = True
...
while(1):
    ...
    classToVis = cv2.getTrackbarPos('ClassToVisualize','Main')
    octavePreset = cv2.getTrackbarPos('Quality','Main')
    sigmaBilateral = cv2.getTrackbarPos('Refinement','Main')

    if cv2.getTrackbarPos('Draw','Main') == 1:
        # Start Drawing, continue to display image, stop on condition that start trackbar goes back to 0
        gen_image = deepdraw(net, gen_image, octaves, focus=classToVis, random_crop=True, visualize=True)
